...
base::FilePath GetPrimaryDisplayCardPath() {
  ...
  for (int i = 0; /* end on first card# that does not exist */; i++) {
    ...
    LOG(INFO) << "trying card_path " << card_path;

    if (access(card_path.c_str(), F_OK) != 0) {
      LOG(INFO) << "Can't access that card";
      break;
    }
    ...
    if (fd < 0) {
      LOG(INFO) << "Failed to open '" << card_path << "'";
      continue;
    }
    ...
    LOG(INFO) << "Failed to get DRM resources for '" << card_path << "'" << " ret " << ret << " " << strerror(errno);
  }
...
DrmDisplayHostManager::DrmDisplayHostManager(
      ...
      weak_ptr_factory_(this) {
  {
    ...
    LOG(INFO) << "Initializing primary drm device";
    if (!primary_drm_device_handle_->Initialize(primary_graphics_card_path_)) {
      LOG(FATAL) << "Failed to open primary graphics card";
      return;
    }
    LOG(INFO) << "DONE Initializing primary drm device";    
...
void DrmDisplayHostManager::ProcessEvent() {
  while (!event_queue_.empty() && !task_pending_) {
    ...
    LOG(INFO) << "Got display event " << kDisplayActionString[event.action_type]
            << " for " << event.path.value();    
...
void DrmDisplayHostManager::OnChannelEstablished(int host_id) {
  ...
  LOG(INFO) << "this " << this << " primary_drm_device_handle " << handle;
  if (!handle) {
    base::ThreadRestrictions::ScopedAllowIO allow_io;
