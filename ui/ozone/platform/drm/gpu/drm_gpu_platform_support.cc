...
void DrmGpuPlatformSupport::SetDelegate(DrmGpuPlatformSupportDelegate* delegate) {
  DCHECK(!delegate_);
  delegate_ = delegate;
}
...
void DrmGpuPlatformSupport::OnConfigureNativeDisplay(
  ...
  DCHECK(delegate_);
  delegate_->DisplayConfigured(id, display_manager_->ConfigureDisplay(id, mode_param, origin));
}
