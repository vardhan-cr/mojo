...
void ScreenManager::AddDisplayController(const scoped_refptr<DrmDevice>& drm,
                                         uint32_t crtc,
                                         uint32_t connector) {
  LOG(INFO) << "ScreenManager::AddDisplayController";
...
bool ScreenManager::ConfigureDisplayController(
  ...
  LOG(INFO) << "ScreenManager::ConfigureDisplayController";
  bool status =
...
void ScreenManager::UpdateControllerToWindowMapping() {
  LOG(INFO) << "ScreenManager::UpdateControllerToWindowMapping";
  ...
  for (HardwareDisplayController* controller : controllers_) {
    LOG(INFO) << "got controller " << controller;    
    if (controller->IsDisabled())
      continue;
    
    DrmWindow* window = FindWindowAt(
        gfx::Rect(controller->origin(), controller->GetModeSize()));
    LOG(INFO) << "FOund window " << window << " rect " << gfx::Rect(controller->origin(), controller->GetModeSize()).ToString();
    if (!window)
      continue;
  ...
  for (auto pair : window_map_) {
    LOG(INFO) << "Calling window SetController";
