...
EgltestWindow::EgltestWindow(PlatformWindowDelegate* delegate,
  ...
  delegate_->OnAcceleratedWidgetAvailable(
    static_cast<gfx::AcceleratedWidget>(window_id_), 1.f);
...
void EgltestWindow::MoveCursorTo(const gfx::Point& location) {
  event_factory_->WarpCursorTo(static_cast<gfx::AcceleratedWidget>(window_id_), location);
}
...
class SurfaceOzoneEgltest : public SurfaceOzoneEGL {
 public:
  SurfaceOzoneEgltest(ShimNativeWindowId window_id,
                      LibeglplatformShimLoader* eglplatform_shim)
      : eglplatform_shim_(eglplatform_shim) {
    native_window_ = static_cast<EGLNativeWindowType>(
      eglplatform_shim_->ShimGetNativeWindow(window_id));
  }
  ~SurfaceOzoneEgltest() override {
    bool ret = eglplatform_shim_->ShimReleaseNativeWindow(
      static_cast<ShimEGLNativeWindowType>(native_window_));
    DCHECK(ret);
  }
...
EGLNativeDisplayType SurfaceFactoryEgltest::GetNativeDisplay() {
  DCHECK(thread_checker_.CalledOnValidThread());
  return reinterpret_cast<EGLNativeDisplayType>(eglplatform_shim_->ShimGetNativeDisplay());
}
...
scoped_ptr<SurfaceOzoneEGL> SurfaceFactoryEgltest::CreateEGLSurfaceForWidget(
  ...
  return make_scoped_ptr<SurfaceOzoneEGL>(
      new SurfaceOzoneEgltest(static_cast<ShimNativeWindowId>(widget), eglplatform_shim_));
}
