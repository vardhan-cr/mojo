...
GLSurfaceOzoneSurfacelessSurfaceImpl::GLSurfaceOzoneSurfacelessSurfaceImpl(
    ...
    AcceleratedWidget widget,
    const gfx::SurfaceConfiguration& requested_configuration)
    : GLSurfaceOzoneSurfaceless(ozone_surface.Pass(), widget, requested_configuration),
      fbo_(0),
      current_surface_(0) {
...
scoped_refptr<GLSurface> GLSurface::CreateViewGLSurface(
  ...
  if (window != kNullAcceleratedWidget) {
    ...
    scoped_refptr<GLSurface> surface = new GLSurfaceStub(requested_configuration);
    if (surface->Initialize())
      return surface;
  }
  return NULL;
...
scoped_refptr<GLSurface> GLSurface::CreateOffscreenGLSurface(
  ...
  switch (GetGLImplementation()) {
    case kGLImplementationOSMesaGL: {
      scoped_refptr<GLSurface> surface(
          new GLSurfaceOSMesa(OSMesaSurfaceFormatBGRA, size, requested_configuration));
      if (!surface->Initialize())
        return NULL;
